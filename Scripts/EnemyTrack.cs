...
public class EnemyTrack : EnemyMovement
{
    [SerializeField]
    ...
    public override void FixedUpdate()
    {
        ...
        {
            if (coll.tag == "Player" && (Vector3.Distance(coll.GetComponent<Transform>().position, transform.position) < minDistance || minDistance == -1))
            {
        ...
        if (closest)
        {
            transform.position = Vector2.MoveTowards(transform.position, closest.position, speed * speedUp * Time.fixedDeltaTime);
        }
        else
        {
            base.FixedUpdate();
